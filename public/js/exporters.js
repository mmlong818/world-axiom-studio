...
function sourceLayerBand(seed) {
  ...
  const layers = reconstructing
    ? ['原文／明确事实', '可追溯推断', '争议并列', '新增设计隔离', '未知保留']
    : ['概览', '运转', '地方', '历史', '生活', '名称'];
  return `<section class="source-layer-band" aria-label="${reconstructing ? '来源分层' : '成书路径'}">
    <span>${reconstructing ? '来源分层' : '成书路径'}</span>
...
function wikiBody(data, forExport = false) {
  ...
  return `<article class="wiki-document">
    ...
    <section class="wiki-intro">
      ...
      <div><span>证据状态</span><strong>${seed.construction_mode === 'reconstruct' ? `${seed.evidence?.confirmed?.length || 0} 项明确 · ${seed.evidence?.unknowns?.length || 0} 项未知` : '创造模型'}</strong></div>
    </section>
    ${worldBookToc()}
...
export function exportData(state) {
  return {
    ...
    skin: state.skin,
    triage: state.triage,
    source_dossier: state.sourceDossier,
...
export function downloadDeliverable(kind, state) {
  ...
  const data = { seed: state.selectedSeed, world: state.world, audit: state.audit, summary: state.summary, art: state.art };
