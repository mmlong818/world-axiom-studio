const SYSTEM = `你是“万象铸界”的世界研究者、世界建筑师与设定编辑。你只完成当前阶段指定的职责，不提前代替其他阶段工作。
...
8. 使用现代、自然、直接的中文。专名首次出现时立即解释；不要用古籍腔、报告腔、抽象名词堆叠或连续四字短语冒充深度。
...
硬性要求：
- overview 使用180至260个中文字符、3至5句自然白话。直接说明“准备怎样呈现”，不要写完整百科内容。
...
function canonPrompt(payload) {
  return `本次阶段：把选定方向建立为唯一的世界正典。正典是后续所有正文、Wiki、简版和图片共用的事实模型，不是文章，也不是待填写清单。
...
const sectionTasks = {
  L1: `写“# 一眼看懂这个世界”和“# 世界如何运转”。先用普通语言建立整体认识，再解释 3至4 条最重要规律。每条规律都说明现象、边界和普通人能看到的后果。总长约 900至1300 个中文字符。`,
  L2: `写“# 地方与彼此关系”和“# 历史为何形成今天”。只介绍正典中理解整体必须知道的地方，先说清空间、交通、资源或权力关系；历史只选择真正改变今天格局的转折，并说明今天仍可见的后果。总长约 1100至1500 个中文字符。`,
  L3: `写“# 人们怎样生活”。解释主要共同体、制度与生计，再通过普通人的工作日自然串起食物、住宅、交易、出行、教育、治疗、礼法和危险。所有内容必须能追溯到正典规律。总长约 1000至1400 个中文字符。`,
  L4: `写“# 重要名称与查阅条目”。只为正典中理解世界不可缺少的实体建立 8至10 个条目；每个条目说明它是什么、为何重要、与谁有关，并建立地方、历史、人群、制度和事物的交叉索引。总长约 900至1300 个中文字符。`,
};
...
正典和 source_continuity 中的原著名称必须原样使用，不得为同一个对象另起一套名称。已有作品模式下，本部分末尾必须写“## 原著承接”，用2至6条简短项目说明本节复现了哪些原著地点、人物、族群或事件，以及新增内容填补了哪个缺口；这不是内部字段，而是读者可见的来源说明。只输出可直接收入世界之书的中文 Markdown。每节先给结论，再解释原因和例子。不要输出代码围栏。`;
}
...
只修改审计指出的问题，保留原著专名、事件、时间顺序、source_continuity 和已经成立的扩展。不得用新名称覆盖原著对象。把生涩长句拆开，把抽象词换成普通说法，合并无解释价值的琐碎条目。只输出修补后的完整中文 Markdown。`;
}
...
世界名称必须原样使用；关键地点、人群、制度和历史不得自行改名。已有作品模式必须明确主世界、次世界、时空接入点，并保留最重要的原著地点、人物或族群和事件名称；扩展内容要说明补足了什么。依次回答：这是怎样的世界、怎样运转、哪些地方和人最重要、历史为什么形成今天、普通人怎样生活、需要记住哪些关键名称。总长 1000至1500 个中文字符，最多保留 12 个关键专名。只写具体世界内容，不写设计方法。`;
}
