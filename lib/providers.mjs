import { callClaudeCli } from './claude-cli.mjs';

const DEFAULT_TIMEOUT_MS = 180_000;
const MAX_DOWNLOADED_IMAGE_BYTES = 20 * 1024 * 1024;
const NETWORK_RETRY_DELAYS_MS = [0, 400, 1_200];

export const providerPresets = {
  openrouter: { label: 'OpenRouter', protocol: 'openai', apiStyle: 'chat', tokenParameter: 'auto', systemRole: 'system', baseUrl: 'https://openrouter.ai/api/v1', model: '' },
  'claude-cli': { label: 'Claude CLI（本机订阅）', protocol: 'anthropic', transport: 'claude-cli', apiStyle: 'cli', tokenParameter: 'cli', baseUrl: '', model: 'sonnet' },
...
function shouldRetryNetworkError(error) {
  ...
  if (code === 'ECONNREFUSED' || code.includes('CERT') || code.includes('TLS')) return false;
  return !code || ['UND_ERR_CONNECT_TIMEOUT', 'UND_ERR_SOCKET', 'ECONNRESET', 'EPIPE', 'ETIMEDOUT', 'EAI_AGAIN', 'ENOTFOUND'].includes(code);
}

function describeNetworkError(error, url, attempts) {
  const code = networkErrorCode(error);
  const host = new URL(url).host;
  const retried = attempts > 1 ? `，已自动尝试 ${attempts} 次` : '';
  if (error?.name === 'TimeoutError' || error?.name === 'AbortError') return new Error('模型响应超时，请重试或换用更快的模型。');
  if (code === 'UND_ERR_CONNECT_TIMEOUT' || code === 'ETIMEDOUT') {
    return new Error(`连接模型端点 ${host} 超时${retried}。请确认代理或网络可用。`);
  }
  if (code === 'ECONNREFUSED') {
    return new Error(`模型端点 ${host} 拒绝连接。请检查端点地址和本地模型服务是否已经启动。`);
  }
  if (code === 'ENOTFOUND' || code === 'EAI_AGAIN') {
    return new Error(`无法解析模型端点域名 ${host}${retried}。请检查端点地址、DNS 或代理设置。`);
  }
  if (code === 'UND_ERR_SOCKET' || code === 'ECONNRESET' || code === 'EPIPE') {
    return new Error(`模型连接在传输中断开${retried}。已保留当前内容，可以从本步骤继续。`);
  }
  const detail = code || String(error?.message || 'fetch failed').slice(0, 120);
  return new Error(`无法连接模型端点 ${host}${retried}（${detail}）。请检查网络或代理设置。`);
}

async function fetchWithNetworkRetry(url, init, timeoutMs) {
  let lastError;
  for (let attempt = 0; attempt < NETWORK_RETRY_DELAYS_MS.length; attempt += 1) {
    if (NETWORK_RETRY_DELAYS_MS[attempt]) {
    ...
    try {
      ...
      lastError = error;
      if (!shouldRetryNetworkError(error) || attempt === NETWORK_RETRY_DELAYS_MS.length - 1) {
        throw describeNetworkError(error, url, attempt + 1);
      }
    }
  }
  throw describeNetworkError(lastError, url, NETWORK_RETRY_DELAYS_MS.length);
}
...
export async function callTextModel(input, system, prompt, { onTextDelta } = {}) {
  ...
  if (config.transport === 'claude-cli' || config.protocol === 'claude-cli') return callClaudeCli(config, system, prompt, { onTextDelta });
